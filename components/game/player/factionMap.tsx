...
const getPlayerItemCoordinate = (
  ...
  const targetCoordinates: DistrictCoordinate = coordinates
    .find((coordinate) => coordinate.id === item.districtId);
  return {
        ...
        <>
            {player.occupations.map((occupation) => {
              if (occupation.active) {
                ...
                const rotation = city.cityBlocks.flat()
                  .find((block) => block.tile.id === occupation.districtId)?.tile.rotation;
                return (
            ...
            }
            {
                player.blocks.map((block) => {
                  ...
                  const numberOfBlocks = getObjectsInDistrict(block.districtId, { player })
                    .blocks.length;
                  return (
                        <BlockPiece
                            key={block.id}
