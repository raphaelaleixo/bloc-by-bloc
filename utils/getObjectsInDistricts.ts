...
export function getObjectsInDistrict(
  districtId: number,
  {
    ...
    players,
    player,
  }: { police?: Police; players?: Player[]; player?: Player },
): ObjectsInDistrict {
  ...
  if (police) {
    ...
    objects.policeBlocks = police.blocks.filter(
      (block) => block.districtId === districtId,
    );
  }
  if (players) {
    let blocks = [];
    ...
    players.forEach((playerInLoop) => {
      blocks = [
        ...blocks,
        ...playerInLoop.blocks.filter(
          (block) => block.districtId === districtId,
        ),
      ];
      occupations = [
        ...occupations,
        ...playerInLoop.occupations.filter(
          (occupation) => occupation.districtId === districtId && occupation.active,
        ),
      ];
    });
    ...
    objects.blocks = player.blocks.filter(
      (block) => block.districtId === districtId,
    );
    objects.occupations = player.occupations.filter(
      (occupation) => occupation.districtId === districtId && occupation.active,
    );
  }
  return objects;
